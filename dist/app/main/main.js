...
const app_services_js_1 = require("../backend/app-services.js");
const { app, BrowserWindow, ipcMain } = require("electron");
const projectRoot = node_path_1.default.resolve(__dirname, "../../..");
const rendererDir = node_path_1.default.join(projectRoot, "app", "renderer");
let mainWindow = null;
let services = null;
function createWindow() {
    ...
    void mainWindow.loadFile(node_path_1.default.join(rendererDir, "index.html"));
}
app.whenReady().then(async () => {
    const databaseDir = process.env.BAR_DB_DIR || node_path_1.default.join(projectRoot, "app", "database");
    services = await (0, app_services_js_1.createAppServices)(databaseDir);
    createWindow();
});
...
function getServices() {
    if (!services)
        throw new Error("Servicos ainda nao inicializados.");
    return services;
}
...
function relatorioPdfHtml(relatorio) {
    const cards = [
...
ipcMain.handle("comanda:adicionar", async (_event, id, codigo, quantidade, precoUnitario) => getServices().comandaService.adicionarItem(id, codigo, quantidade, precoUnitario));
ipcMain.handle("comanda:editar-preco-item", async (_event, id, codigo, precoUnitario) => getServices().comandaService.editarPrecoItem(id, codigo, precoUnitario));
ipcMain.handle("comanda:remover", async (_event, id, codigo, quantidade) => getServices().comandaService.removerItem(id, codigo, quantidade));
ipcMain.handle("comanda:adicionar-mais", async (_event, id, codigo, quantidade) => getServices().comandaService.adicionarMaisItem(id, codigo, quantidade));
ipcMain.handle("comanda:cancelar", async (_event, id) => getServices().comandaService.cancelar(id));
ipcMain.handle("comanda:pagar-itens", async (_event, id, itensSelecionados, metodoPagamento, valorRecebido, valorPago) => getServices().comandaService.pagarItens(id, itensSelecionados, metodoPagamento, valorRecebido, valorPago));
ipcMain.handle("comanda:dividir", async (_event, id, pessoas) => getServices().comandaService.dividirConta(id, pessoas));
ipcMain.handle("fiado:quitar", async (_event, comandaId, dataFiado, metodoPagamento, valorRecebido) => getServices().pagamentoService.quitarFiado(comandaId, dataFiado, metodoPagamento, valorRecebido));
ipcMain.handle("fiado:cancelar", async (_event, comandaId, dataFiado) => getServices().pagamentoService.cancelarFiado(comandaId, dataFiado));
ipcMain.handle("relatorio:periodo", async (_event, inicio, fim) => getServices().pagamentoService.gerarRelatorioPorPeriodo(inicio, fim));
ipcMain.handle("relatorio:exportar-pdf", async (_event, inicio, fim) => {
    ...
    const filePath = node_path_1.default.join(currentServices.fileService.relatoriosDir, `relatorio_${safeInicio}_${safeFim}.pdf`);
    await (0, promises_1.writeFile)(filePath, pdf);
    return { filePath };
});
ipcMain.handle("comanda:fechar", async (_event, id, metodoPagamento, valorRecebido) => getServices().comandaService.fechar(id, metodoPagamento, valorRecebido));
